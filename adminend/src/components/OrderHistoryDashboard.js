  ...
  return (
    <DashboardShell activeItem="Order History">
      <div className="rounded-[28px] border border-neutral-200 bg-white px-6 py-7 shadow-lg shadow-main/5 md:px-8">
        ...
        <h1 className="mt-3 text-3xl font-black tracking-tight text-main md:text-4xl">Order History</h1>
      ...
      <div className="mt-8 grid gap-5 md:grid-cols-2 xl:grid-cols-4">
        {summaryCards.map((card) => (
          <article key={card.title} className={`relative overflow-hidden rounded-[24px] border border-neutral-200 bg-white p-7 shadow-lg shadow-main/5 ring-1 ${card.ring}`}>
            <div className={`absolute inset-x-0 top-0 h-1.5 bg-gradient-to-r ${card.accent}`} />
            <p className="text-base font-extrabold text-slate-500">{card.title}</p>
            <p className="mt-5 text-4xl font-black tracking-tight text-main">{card.value}</p>
              ...
              {orders.map(([id, customer, date, payment, paymentStatus, total, orderStatus, billStatus]) => (
                <tr key={id} className="border-b border-slate-100 last:border-b-0">
                  ...
                  <td className="px-10 py-7 text-center">
                    <button type="button" className="h-11 rounded-2xl bg-main px-6 text-sm font-black text-white shadow-md shadow-main/20 transition hover:bg-mainHover">View</button>
                  </td>
                </tr>
              ))}
            </tbody>
